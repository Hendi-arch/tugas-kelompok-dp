from store_utils import gen_product_id, title, get_truly_words, lines, get_product_action
...
class StoreMenus:
    ...
    def show_products(self):
        ...
        print('\n')
        for index, product in enumerate(self.__main_courses +
                                        self.__beverages):
    ...
    def show_main_course(self):
        ...
        for product in self.__main_courses:
            print(
                f"{product.get_index()}. {product.get_product_name()} - Rp. " +
                "{:,.2f}".format(product.get_price()) +
                f" {product.get_icon()}")
...
change the product price, change the product quantity, or change the product name.""")
    ...
    def pick_main_courses(self):
        ...
        selected_menu = list(
            filter(lambda item: item.get_index() == menu_id,
                   self.__main_courses))
        ...
        if len(selected_menu) > 0:
            ...
            menu_exist = list(
                filter(
                    lambda item: item.get_product_id() == selected_menu[0].
                    get_product_id(), self.__selected_menus.values()))

            if len(menu_exist) <= 0:
                selected_menu[0].set_qty(order_qty)
                self.__selected_menus[
                    selected_menu[0].get_product_id()] = selected_menu[0]
            else:
                menu_exist[0].set_qty(menu_exist[0].get_qty() + order_qty)
                self.__selected_menus[
                    menu_exist[0].get_product_id()] = menu_exist[0]
            ...
            while True:
                try:
                    ...
                    if order_qty > 0:
                        ...
                        answer: str = str(
                            input(
                                "Are you sure you don't want to order a drink ? "
                            ))
